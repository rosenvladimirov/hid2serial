...
{
    //
    // Discard everything in the buffer. We update Head before Tail so
    // any concurrent reader observing a partially-updated state still
    // sees Head <= Tail (i.e. an apparently-empty or apparently-full
    // buffer, never a state where it tries to read past Tail).
    //
    Self->Head = Self->Base;
    Self->Tail = Self->Base;
}
    ...
    )
{
    ...
    if (bytesToCopy)
    {
        ...
        if ((Self->Tail + bytesToCopy) > Self->End)
        {
            ...
            //
            // Publish the bytes BEFORE advancing the tail. On x64 the
            // store-store ordering is implicit (TSO); on ARM64 it is
            // not — without this barrier the consumer can observe the
            // new tail and read stale bytes from the ring slot.
            //
            MemoryBarrier();
        ...
        else
        {
            ...
            //
            // Same store-store fence as the wrap-around branch — see
            // comment above for the ARM64 reasoning.
            //
            MemoryBarrier();
    ...
    )
{
    ...
    *BytesCopied = DataSize;
    ...
    //
    // Acquire-side fence — make sure we read the producer's bytes
    // AFTER seeing the new tail (RingBufferGetAvailableData read it
    // a few lines above). On ARM64 a missing barrier here lets the
    // CPU reorder the byte loads ahead of the tail-snapshot load,
    // pulling stale data out of the ring slots.
    //
    MemoryBarrier();
