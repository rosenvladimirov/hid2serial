...
static NTSTATUS
DeviceQueryHardwareId(
    _In_  PWDFDEVICE_INIT   DeviceInit,
    _Out_writes_z_(BufferCch) PWCHAR Buffer,
    _In_  ULONG             BufferCch
    )
{
    NTSTATUS status;
    ULONG    actualLen   = 0;
    ULONG    bufferBytes = BufferCch * sizeof(WCHAR);
...
Routine Description:
    ...
    AddDevice for one half of a pair (side A or side B). Builds the
    WDFDEVICE, registers in the driver-wide pair table so the peer
    can find us, then sets up the COM port symlink and IO queues
    exactly like Phase 1 did for a single port.

--*/
{
    NTSTATUS        status;
    PDEVICE_CONTEXT deviceContext;
    PAIR_SIDE       side = PAIR_SIDE_NONE;
    WCHAR           hwId[128];

    UNREFERENCED_PARAMETER(Driver);

    (void)DeviceQueryHardwareId(DeviceInit, hwId, ARRAY_SIZE(hwId));

    status = DeviceCreate(Driver, DeviceInit, &deviceContext);
    if (!NT_SUCCESS(status)) {
        return status;
    }

    //
    // Register in the pair table. A failure here (e.g. side already
    // taken) leaves the device usable but unpaired — writes pile up
    // in our own ring with no reader, reads block until a peer
    // attaches. We don't fail the device because the user can still
    // observe and replace the duplicate.
    //
    status = PairAttachChild(deviceContext, hwId, &side);
    if (!NT_SUCCESS(status)) {
        Trace(TRACE_LEVEL_INFO,
            "ChildEvtDeviceAdd: PairAttachChild status 0x%x (running unpaired)",
            status);
    ...
    )
{
    ...
    //
    // Drop our half of the pair so the peer (or a re-installed
    // replacement child) can attach without "already attached".
    //
    PairDetachChild(deviceContext);
