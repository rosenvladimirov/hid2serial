...
//
// PAIR_SIDE comes from pair.h (included before this file in
// internal.h). The QUEUE_CONTEXT struct body is in queue.h, which is
// included after this header — we use a struct-pointer field here so
...
typedef struct _DEVICE_CONTEXT
{
    ...
    //
    // Phase 2 — pair linkage. Side identifies which half of the pair
    // this child is; QueueContext is the back-pointer to the per-side
    // queue state, populated by QueueCreate. The peer's data is at
    // PairGetPeer(this)->QueueContext->RingBuffer / ReadQueue.
    //
    PAIR_SIDE       Side;

    struct _QUEUE_CONTEXT *QueueContext;
